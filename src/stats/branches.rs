...
pub fn analyze_branches(
    ...
    let base_oid = base_ref.target().ok_or_else(|| anyhow::anyhow!("Base branch has no target OID"))?;
...
fn determine_base_branch<'a>(repo: &'a Repository, base_branch: Option<&'a str>) -> Result<git2::Reference<'a>> {
...
fn compute_branch_metrics(
    ...
    let branch_oid = branch_ref.target().ok_or_else(|| anyhow::anyhow!("Branch has no target OID"))?;
    ...
    let (commits, churn_adds, churn_dels, merge_ratio) = compute_branch_activity(
        repo,
        branch_oid,
        base_oid,
        since,
...
mod tests {
    ...
    fn create_commit(repo: &Repository, message: &str, parent: Option<&git2::Commit>) -> Result<git2::Oid> {
        let sig = Signature::new("Test User", "test@example.com", &Time::new(1000000, 0))?;
    ...
    fn test_merge_ratio_calculation() -> Result<()> {
        ...
        let result = compute_branch_activity(
            &repo,
            head_oid,
            head_oid, // Same oid means no commits between
            None,
            None,
            false,
        );
