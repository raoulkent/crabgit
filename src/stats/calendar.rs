...
mod tests {
    ...
    fn test_calendar_heatmap_serialization() {
        ...
        assert!(json_result.is_ok(), "CalendarHeatmap should serialize to JSON");
        
        let json_str = json_result.unwrap();
        assert!(json_str.contains("matrix"), "JSON should contain matrix field");
    }
    ...
    fn test_calendar_large_values() {
        ...
        let json_result = serde_json::to_string(&heatmap);
        assert!(json_result.is_ok(), "Should handle large values in serialization");
    }
    ...
    fn test_calendar_weekday_mapping() {
        ...
        for (weekday, expected_index) in test_cases {
            ...
            assert_eq!(array_index, expected_index, 
                "Weekday {:?} should map to array index {}", weekday, expected_index);
        }
    }
    ...
    fn test_calendar_time_consistency() {
        ...
        for ts in test_timestamps {
            ...
            assert!(iso_weekday < 7, "Weekday index should be < 7, got {}", iso_weekday);
    ...
    fn test_time_parsing_integration() {
        ...
        for test_case in test_cases {
            ...
            if let Some(timestamp) = result {
                ...
                assert!(timestamp > 0, "Parsed timestamp should be positive for: {}", test_case);
