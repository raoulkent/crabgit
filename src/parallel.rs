    ...
    T: Send + Sync,
{
    ...
    {
        let batches: Vec<_> = self.items.chunks(self.batch_size).collect();

        if self.parallel_config.enabled {
            batches.into_par_iter().flat_map(&processor_fn).collect()
        } else {
            batches.into_iter().flat_map(&processor_fn).collect()
        }
    }

    /// Process batches with progress tracking
    pub fn process_with_progress<R, F, P>(&self, processor_fn: F, progress_fn: P) -> Vec<R>
    where
        R: Send,
    ...
    T: Send + Sync,
{
    ...
    where
        ...
        F: Fn(&[T]) -> R + Send + Sync,
    {
        let windows: Vec<_> = self.items.chunks(self.window_size).collect();
...
mod tests {
    ...
    fn test_par_filter_map() -> Result<()> {
        ...
        let results =
            processor.par_filter_map(numbers, |x| if x % 2 == 0 { Some(x * 2) } else { None });
    ...
    fn test_memory_safe_iterator() {
        ...
        let results = iterator.process_windows(|window| window.iter().sum::<i32>());
