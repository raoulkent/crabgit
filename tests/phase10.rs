...
use gitcrab::cache::{CacheConfig, CommitStatsCache, CachedCommitStats};
use gitcrab::config::{GlobalConfig, ConfigOverrides, ParallelConfig, DebugConfig, parse_duration_to_days};
use gitcrab::telemetry::{Timer, PerfMetrics, DebugLogger};
use gitcrab::parallel::{ParallelProcessor, BatchProcessor, MemorySafeIterator};
...
fn test_parallel_processing() -> Result<()> {
    ...
    let filtered_results = processor.par_filter_map(numbers.clone(), |x| {
        if x % 2 == 0 {
            Some(x * 3)
        } else {
            None
...
fn test_batch_processing() {
    ...
    let processor = BatchProcessor::new(numbers, ParallelConfig {
...
fn test_memory_safe_iterator() {
    ...
    let window_sums = iterator.process_windows(|window| {
        window.iter().sum::<i32>()
    });
    ...
    let sequential_sums = sequential_iterator.process_windows(|window| {
        window.iter().sum::<i32>()
    });
...
fn test_performance_under_load() -> Result<()> {
    ...
    let start = Instant::now();
    let sequential_results = sequential_processor.par_map(large_dataset, |x| {
        (x % 1000) * 2 + 1
    });
